...
Откройте `настройка последовательного запуска.bat`, стрелками выберите аккаунты, Enter добавляет или убирает аккаунт из порядка. Выберите `Save`, чтобы сохранить очередь.

После настройки запускайте `последовательный запуск.bat`. Он выполнит аккаунты в сохраненном порядке.
...
```powershell
Copy-Item templates\example.env .env.acc1
notepad .env.acc1
```
...
- `STICKER_SET_ID`, `STICKER_SET_ACCESS_HASH`, `STICKER_DOC_ID` - сохраненный стикер; если пусто, скрипт попросит отправить стикер в Избранное.
...
- `storage/report.csv` - отчёт при общем режиме `STORAGE_MODE=shared`.
